...
impl GitWorktreeManager {
    ...
    #[must_use]
    pub fn worktrees_root(&self) -> &Path { &self.worktrees_root }
    ...
    pub fn create(&self, name: &str, base: Option<&str>) -> Result<GitWorktree, WorkspaceError> {
        ...
        if path.exists() { return Err(WorkspaceError::AlreadyExists(name.to_owned())); }

        let branch = format!("nexus/{name}");
        let base = base.unwrap_or("HEAD");
        let path_string = path.to_string_lossy().to_string();
        self.git(&["worktree", "add", "-b", branch.as_str(), path_string.as_str(), base])?;
        Ok(GitWorktree { name: name.to_owned(), branch, path })
    }

    /// Allocate one isolated worktree per parallel agent. Partial allocations are
    /// cleaned up if a later worktree cannot be created.
    pub fn allocate_agents(&self, run_name: &str, count: usize, base: Option<&str>) -> Result<Vec<AgentWorkspace>, WorkspaceError> {
        if count == 0 { return Err(WorkspaceError::InvalidAgentCount); }
        validate_name(run_name)?;
        let mut allocated = Vec::with_capacity(count);
        for agent_index in 0..count {
            let name = format!("{run_name}-agent-{}", agent_index + 1);
            match self.create(&name, base) {
                Ok(worktree) => allocated.push(AgentWorkspace { agent_index, worktree }),
                Err(error) => {
                    for workspace in &allocated { let _ = self.remove(&workspace.worktree.name, true); }
                    return Err(error);
                }
            }
        }
                ...
                if let (Some(path), Some(branch)) = (path.take(), branch.take()) {
                    ...
                    if let Ok(relative) = path.strip_prefix(&self.worktrees_root) {
                        if relative.components().count() == 1 {
                            worktrees.push(GitWorktree { name: relative.to_string_lossy().to_string(), branch, path });
                        }
                    }
                }
                continue;
    ...
    pub fn remove(&self, name: &str, force: bool) -> Result<(), WorkspaceError> {
        ...
        if force { self.git(&["worktree", "remove", "--force", path.as_str()])?; }
        else { self.git(&["worktree", "remove", path.as_str()])?; }
        Ok(())
    }

    fn find(&self, name: &str) -> Result<GitWorktree, WorkspaceError> {
        self.list()?.into_iter().find(|worktree| worktree.name == name).ok_or_else(|| WorkspaceError::NotFound(name.to_owned()))
    }
    ...
    fn git_in(&self, directory: &Path, args: &[&str]) -> Result<String, WorkspaceError> {
        let output = Command::new("git").current_dir(directory).args(args).output().map_err(WorkspaceError::Io)?;
...
fn validate_name(name: &str) -> Result<(), WorkspaceError> {
    let valid = !name.is_empty() && !matches!(name, "." | "..") && name.chars().all(|character| character.is_ascii_alphanumeric() || matches!(character, '-' | '_' | '.'));
    if !valid { return Err(WorkspaceError::InvalidName(name.to_owned())); }
    Ok(())
}

#[derive(Debug, thiserror::Error)]
pub enum WorkspaceError {
    #[error("workspace name is invalid: {0}")] InvalidName(String),
    #[error("workspace already exists: {0}")] AlreadyExists(String),
    #[error("workspace not found: {0}")] NotFound(String),
    #[error("parallel agent count must be greater than zero")] InvalidAgentCount,
    #[error("git command failed: {0}")] Git(String),
    #[error(transparent)] Io(#[from] std::io::Error),
}

#[cfg(test)]
mod tests {
    use super::{WorkspaceError, validate_name};
    #[test]
    ...
    #[test]
    fn zero_agents_is_rejected_by_contract() {
        assert!(matches!(WorkspaceError::InvalidAgentCount, WorkspaceError::InvalidAgentCount));
    }
}
