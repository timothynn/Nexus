//! Minimal MCP client primitives for Nexus.
//!
//! The first transport is stdio: Nexus launches a server subprocess and exchanges
//! one JSON-RPC message per line over stdin/stdout. Higher-level code can map
//! discovered MCP tools into the Nexus tool registry without coupling the agent
//! runtime to a particular MCP server.
...
impl StdioMcpClient {
    pub async fn connect(command: &McpServerCommand) -> Result<Self, McpError> {
        ...
        let stdin = child
            .stdin
            .take()
            .ok_or_else(|| McpError::Protocol("server stdin unavailable".to_owned()))?;
        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| McpError::Protocol("server stdout unavailable".to_owned()))?;
        Ok(Self {
            child,
            stdin,
    ...
    async fn request(&mut self, method: &str, params: Value) -> Result<Value, McpError> {
        ...
        loop {
            let Some(line) = self.stdout.next_line().await.map_err(McpError::Io)? else {
...
#[cfg(test)]
mod tests {
    use super::McpServerCommand;

    #[test]
    fn command_starts_without_arguments() {
        let command = McpServerCommand::new("server");
